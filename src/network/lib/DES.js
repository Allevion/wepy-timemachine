...
(function() {
  const C = (typeof window === 'undefined') ? require('./Crypto').Crypto : window.Crypto
    ...
    // Shortcuts
  let util = C.util, charenc = C.charenc, UTF8 = charenc.UTF8
  ...
  KeySchedule.prototype._initialiseKeys = function(key) {
    ...
    for (i = 0; i < 16; i++) {
      ...
      const s = KeySchedule.keyShifts[i]
      for (var j = 0; j < 28; j++) {
        if (bits1[j + s]) {
  ...
  State.prototype._exchangeLR = function(v, m) {
    const t = ((this.lhs >> v) ^ this.rhs) & m
    this.rhs ^= t
    this.lhs ^= (t << v)
  }

  State.prototype._exchangeRL = function(v, m) {
    const t = ((this.rhs >> v) ^ this.lhs) & m
    this.lhs ^= t
    this.rhs ^= (t << v)
  }
  ...
  State.prototype.initialPerm = function(message, offset) {
    const input = message.slice(offset, offset + 8)
  ...
  State.prototype.round = function(k) {
    let r = this.rhs, l = this.lhs
    let f = 0
    for (let i = 0; i < 8; i++) {
      const v = (r ^ k[i]) & State.SBOX_MASK[i]
      f += State.SBOX[i][v]
    }
